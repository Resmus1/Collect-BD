...
def run_parser_for_region(region, main_category, subcategory, attempt=1):
    ...
    old_data = read_json_data(
        f"output/{region}/{main_category}/{subcategory}.json")
    existing_cards = {
        (x["name"], x["address"]): x for x in old_data if x.get("address")
    }
    collect_data = []
    count_cards = 0
    ...
    logger.info(
        f"💾 Сохраняется {len(existing_cards)} карточек в {region}/{main_category}/{subcategory}.json")
    write_json_data(
        list(existing_cards.values()),
        Path("output") / region / main_category / f"{subcategory}.json"
    )
        ...
        with Pool(processes=num_processes) as pool:
            ...
            for i, (r, args) in enumerate(zip(results, filtered_args_list)):
                ...
                try:
                    ...
                    if completed_regions.get(key) is not True:
